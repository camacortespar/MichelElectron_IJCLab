...
vector<vector<pair<vector<vector<double>>, int const>>> ListXYZ(string rootfilelabel)
{
  ...
  for(gallery::Event ev(filename); !ev.atEnd(); ev.next()){
    ...
    auto particles = ev.getValidHandle<vector<recob::PFParticle>>(module);
    size_t npart   = particles->size();        
    const art::FindManyP<recob::SpacePoint> findSpacePoints(particles, ev, module);     //here are the space point of each particle
    ...
    for(size_t i = 0, sz = npart; i != sz; ++i){
      ...
      vector<vector<double>> PtsXYZ(SPs.size());        //position vector
            
      for(size_t j = 0, sz = SPs.size(); j != sz; ++j){

      double X = SPs.at(j)->XYZ()[0];  PtsXYZ.at(j).push_back(X);
      double Y = SPs.at(j)->XYZ()[1];  PtsXYZ.at(j).push_back(Y);
      double Z = SPs.at(j)->XYZ()[2];  PtsXYZ.at(j).push_back(Z);

      }

      pair<vector<vector<double>>, int const> p(PtsXYZ , pdg);      //space points linked with pdgcode
      ShXYZ.push_back(p);
...
)
{
  ...
  Analysis(ListXYZ(filenames[0]).at(event_i-1), event_i);
